...
class Settings(BaseSettings):
    ...
    LANGSMITH_TRACING: bool = (
        True  # Enable LangChain native tracing to LangSmith when API key is set
    )
    LANGSMITH_OTEL_EXPORT: bool = (
        True  # Export OpenTelemetry spans to LangSmith OTEL endpoint when API key is set
    )
    LANGSMITH_ENDPOINT: str = (
        "https://api.smith.langchain.com"  # LangSmith API base (use for self-hosted)
    )
